...
import type { CreateAssetInput, CreateAssetResponse } from "@/schemas/asset";
...
function AssetCreateForm({ models, offices, rooms }: AssetCreateFormProps) {
    const router = useRouter();
    const [state, action, pending] = useActionState(
        async (_: CreateAssetResponse | null, payload: CreateAssetInput) => {
    ...
    useEffect(() => {
        ...
        if (!state.success) {
            if (state?.errors) {
                ...
                toast.error(
                    state?.message || "Terjadi kesalahan saat membuat aset."
                );
            }
        }
        ...
        if (state.success) {
            ...
            router.push(`/assets/${state.data?.id}`);
        }
    }, [state, form, router]);
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Isi informasi aset perangkat di bawah ini.
                </CardDescription>
            </CardHeader>
